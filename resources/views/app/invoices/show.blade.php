                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-gradient-to-br from-gray-900 to-gray-950 rounded-xl border border-gray-700/50 p-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <!-- Send Button Outside Modal -->
                            <form action="{{ route('app.invoices.send', $invoice) }}" method="POST" class="w-full">
                                @csrf
                                <button type="submit" class="w-full px-4 py-3 bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-lg font-semibold text-gray-900 hover:shadow-lg shadow-yellow-400/30 transition-all text-sm text-left flex items-center">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                                    </svg>
                                    Versturen (Zonder Preview)
                                </button>
                            </form>
                            ...
                            @if($invoice->status !== 'paid')
                            <form action="{{ route('app.invoices.reminder', $invoice) }}" method="POST" class="w-full">
                                @csrf
                                <button type="submit" class="w-full px-4 py-3 bg-gradient-to-r from-orange-500/10 to-orange-600/10 text-orange-400 border border-orange-500/30 rounded-lg hover:bg-orange-500/20 transition-all text-sm font-semibold text-left flex items-center">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @endphp
                                    @if($daysOverdue > 0)
                                        Verstuur Herinnering ({{ $daysOverdue }} dag(en) achterstallig)
                                    @else
